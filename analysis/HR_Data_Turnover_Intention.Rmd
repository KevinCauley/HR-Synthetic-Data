---
title: "Turnover Intention Modeling | Workforce Analytics"
...
plot_df <- results_all %>%
  ...
  filter(any(.metric == "f_meas" & !is.na(.estimate))) %>%
  ungroup() %>%
  mutate(
    Metric = factor(
      Metric,
...
p <- ggplot(
  plot_df,
  aes(x = Metric, y = reorder(Model_Sampling, .estimate), fill = .estimate)
) +
  ...
  labs(
    ...
    subtitle = "Higher values indicate stronger performance for each evaluation metric",
    x = NULL,
    y = NULL
  ) +
  theme_hr(10) +
...
p <- final_preds %>%
  ...
  labs(
    title = "Predicted Turnover Risk Distribution",
    subtitle = "Employees are scored by their predicted probability of turnover intention",
    x = "Predicted Probability of Turnover",
    y = "Employee Count"
  ) +
  theme_hr(11) +
  theme(
...
p <- ggplot(risk_segments, aes(x = Risk_Level, y = n, fill = Risk_Level)) +
  ...
  labs(
    ...
    subtitle = "Risk tiers translate model scores into retention intervention priorities",
    x = NULL,
    y = "Employee Count"
  ) +
  theme_hr(11) +
