...
# Define directories (single source of truth)
...
continuous_hist_files <- numeric_long %>%
  ...
  purrr::map_chr(function(data){
    ...
    ggsave(filepath, p, width = 6.5, height = 3.5, dpi = 120, bg = "white")
...
likert_outlier_files <- purrr::map_chr(likert_vars, function(var){
  ...
  p <- ggplot(hr_df, aes(x = "", y = .data[[var]])) +
    geom_boxplot(fill = COLORS$primary, alpha = 0.7, width = 0.3, outlier.shape = NA) +
...
if (ncol(corr_input) < 2) {
  stop("Not enough valid variables for correlation matrix")
}
...
```{r 05-pairwise-correlation, echo=TRUE}
...
corr_input <- hr_df %>%
  dplyr::select(dplyr::any_of(pairwise_vars)) %>%
  dplyr::select(where(is.numeric))

corr_input <- corr_input[, colSums(is.na(corr_input)) < nrow(corr_input)]
corr_input <- corr_input[, apply(corr_input, 2, sd, na.rm = TRUE) > 0]

print("Columns used for correlation:")
print(names(corr_input))

if (ncol(corr_input) < 2) {
  stop("Not enough valid numeric variables for correlation")
}

corr_matrix <- stats::cor(
  as.matrix(corr_input),
...
p <- ggcorrplot::ggcorrplot(
  ...
  labs(title = "Key Pairwise Correlations") +
  theme(
...
```{r 05-pairwise-scatter, echo=TRUE}

fig_path <- file.path(FIG_DIR, "pairwise_scatter.png")
...
sample_df <- hr_df %>%
  ...
  dplyr::sample_n(min(300, nrow(.)))

p <- ggplot(sample_df,
            aes(x = engagement_level, y = job_satisfaction)) +
  ...
  labs(
    title = "Engagement vs Job Satisfaction",
    x = "Engagement Level",
    y = "Job Satisfaction"
  ) +
...
fig_path <- file.path(PLOT_DIR, "turnover_by_department.png")
...
skew_df <- hr_df %>%
  dplyr::select(where(is.numeric)) %>%
  dplyr::select(where(~ sum(!is.na(.)) > 0)) %>%  # remove all-NA columns
  dplyr::summarise(
    dplyr::across(
      everything(),
  ...
  dplyr::mutate(
    Variable = clean_label(Variable),
    Skew_Type = dplyr::case_when(
      Skewness < -1 ~ "Highly Left-Skewed",
      Skewness < -0.5 ~ "Moderately Left-Skewed",
      Skewness <= 0.5 ~ "Approximately Symmetric",
      Skewness <= 1 ~ "Moderately Right-Skewed",
      TRUE ~ "Highly Right-Skewed"
    )
  )

# 🔥 SAFETY CHECK
if (nrow(skew_df) == 0) {
  stop("Skewness dataframe is empty")
}
...
fig_path <- file.path(FIG_DIR, "categorical_imbalance.png")
