...
```{css, echo=FALSE}
/* ===== FIX PAGE WIDTH + REMOVE LEFT GAP ===== */
.main-container {
  max-width: 1200px;
  margin-left: 280px;   /* match TOC width */
  margin-right: auto;
  overflow-x: hidden;
}
...
/* ===== REMOVE MASSIVE WHITE SPACE UNDER WIDGETS ===== */
.html-widget {
  margin-bottom: 0px !important;
  padding-bottom: 0px !important;
}

/* ===== FIX SLICK CAROUSEL HEIGHT ===== */
.slick-slider,
.slick-list,
.slick-track {
...
/* ===== REMOVE EXTRA GAP BETWEEN SLIDES ===== */
.slick-slide {
...
continuous_hist_files <- numeric_long %>%
  ...
  purrr::map_chr(function(data){
    ...
    p <- ggplot(data, aes(x = Value)) +
      ...
      geom_density(color = COLORS$accent, linewidth = 1.2) +
      labs(
  ...
  y = "Density"
    ...
    ggsave(filename = filepath, plot = p, width = 10, height = 6, dpi = 300, bg = "white")
...
likert_plot_files <- purrr::map_chr(likert_vars, function(var){
  ...
  p <- hr_df %>%
    ...
    geom_col(fill = COLORS$primary, width = 0.7) +
...
likert_outlier_files <- purrr::map_chr(likert_vars, function(var){
  ...
  p <- ggplot(hr_df, aes(x = "", y = .data[[var]])) +
    geom_boxplot(fill = COLORS$primary) +
  ...
  ggsave(filename = filepath, plot = p, width = 8, height = 5, dpi = 300, bg = "white")
...
```{r 05-correlation-matrix, fig.width=16, fig.height=14}
numeric_vars <- hr_df %>%
  select(where(is.numeric)) %>%
...
ggsave(filename = file.path(FIG_DIR, "correlation_matrix.png"),
       plot = p, width = 14, height = 12, dpi = 300, bg = "white")
...
```{r 05-strong-correlations, fig.width=10, fig.height=8}
...
ggsave(filename = file.path(FIG_DIR, "top_correlations.png"),
...
ggsave(filename = file.path(FIG_DIR, "pairwise_relationships.png"),
...
ggsave(
  filename = file.path(FIG_DIR, "vif_numeric.png"),
  plot = p,
  width = 12,
  height = 10,
  dpi = 300,
...
ggsave(
  filename = file.path(FIG_DIR, "vif_categorical.png"),
  plot = p,
  width = 12,
  height = 10,
  dpi = 300,
...
```{r 07-vif-categorical-updated-plot, fig.width=9, fig.height=6}
...
p <- ggplot(skew_df, aes(x = Skewness, y = Variable, fill = Skew_Type)) +
  ...
  labs(title = "Skewness of Variables")

ggsave(
  filename = file.path(FIG_DIR, "skewness.png"),
  plot = p,
  width = 12,
  height = 10,
  dpi = 300,
...
# Select numeric variables (excluding IDs if needed)
numeric_vars <- hr_df %>%
  select(where(is.numeric))

# Compute skewness
numeric_imbalance_df <- numeric_vars %>%
...
fig_path <- file.path(FIG_DIR, "numeric_imbalance.png")

p <- ggplot(numeric_imbalance_df,
  ...
  labs(
    title = "Numeric Variable Imbalance (Skewness)",
    x = "Variable",
    y = "Skewness"
  ) +
...
cat_plot_files <- purrr::map_chr(cat_vars_current, function(var){
  ...
  plot_df <- hr_df %>%
    count(!!sym(var)) %>%
    mutate(label = as.character(!!sym(var)))
...
# Extract coefficients FIRST (THIS WAS MISSING)
coef_df <- broom::tidy(log_model) %>%
  filter(term != "(Intercept)") %>%
  mutate(term = clean_label(term)) %>%
  arrange(estimate)
```
...
p <- ggplot(coef_df,
            aes(x = estimate, y = reorder(term, estimate), fill = estimate > 0)) +

  geom_col(width = 0.7, alpha = 0.9) +

  scale_fill_manual(
    values = c("TRUE" = COLORS$primary, "FALSE" = COLORS$accent),
    guide = "none"
  ) +
