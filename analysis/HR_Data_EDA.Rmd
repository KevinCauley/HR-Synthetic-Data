...
The analysis includes exploratory data analysis, multicollinearity diagnostics, and predictive modeling to understand patterns in employee engagement, monitoring practices, and turnover risk.
...
```{r categorical_vif_plot, fig.width=10, fig.height=7, fig.align="center"}
...
```{r 03-skewness-of-num-vars, fig.width=10, fig.height=7, fig.align="center"}
...
skew_df <- skew_df %>%
  mutate(Variable = fct_reorder(Variable, Skewness))

# Plot
ggplot(skew_df, aes(x = Skewness, y = Variable, fill = Skew_Type)) +
...
```{r 03-categorical-distributions, results='asis'}
...
cat_plot_files <- purrr::map(cat_vars_current, function(var) {
  ...
  ggsave(filepath, p, width = 7, height = 4.5, dpi = 150)
...
# Display carousel
slickR(obj = cat_plot_files, slideType = "img", width = "750px", height = "450px") +
  settings(dots = TRUE, arrows = TRUE, infinite = FALSE)

cat("<br><br><br>")   # critical spacing so next plot does not overlap
```

```{r 03-skew-cat-graph, fig.width=9, fig.height=5}
...
cat_imbalance_df %>%
  mutate(Variable = fct_reorder(Variable, Most_Common_Proportion)) %>%
