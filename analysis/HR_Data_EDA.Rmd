...
continuous_hist_files <- numeric_long %>%
  ...
  purrr::map_chr(function(data){
    ...
    ggsave(filepath, p, width = 10, height = 6, dpi = 300, bg = "white")
...
likert_plot_files <- purrr::map_chr(likert_vars, function(var){
  ...
  p <- hr_df %>%
    ...
    # Force correct Likert ordering
    mutate(score = factor(
      !!sym(var),
      levels = sort(unique(!!sym(var)))
    )) %>%
...
likert_outlier_files <- purrr::map_chr(likert_vars, function(var){
  ...
  p <- ggplot(hr_df, aes(x = "", y = .data[[var]])) +
    ...
    # Clean Boxplot with no duplicate outliers
    geom_boxplot(
      fill = COLORS$primary,
      alpha = 0.7,
      width = 0.3,
...
p <- ggplot(top_corr_df, aes(x = Pair, y = Correlation, fill = Direction)) +
  ...
  geom_text(
    ...
    size = 3.5,
    hjust = ifelse(top_corr_df$Correlation > 0, -0.2, 1.2)
  ) +
  ...
  labs(
    ...
    subtitle = "Top 20 relationships by strength",
    x = NULL,
    y = "Correlation"
  ) +
...
p <- ggplot(plot_df,
       aes(x = reorder(department, n), y = n, fill = turnover_intention)) +
...
ggsave(
  filename = file.path(FIG_DIR, "turnover_department.png"),
  plot = p,
  width = 10,
  height = 6,
  dpi = 300,
...
ggsave(
  filename = file.path(FIG_DIR, "most_skewed.png"),
  plot = p,
  width = 12,
  height = 10,
  dpi = 300,
...
cat_plot_files <- purrr::map_chr(cat_vars_current, function(var){
  ...
  plot_df <- hr_df %>%
    count(!!sym(var)) %>%
    mutate(label = clean_label(!!sym(var)))
  ...
  ggsave(filepath, p, width = 9, height = 5.5, dpi = 300, bg = "white")
...
p <- ggplot(coef_df,
       aes(x = reorder(term_clean, estimate), y = estimate, fill = estimate > 0)) +
...
ggsave(
  filename = file.path(FIG_DIR, "logistic_importance.png"),
  plot = p,
  width = 10,
  height = 6,
  dpi = 300,
