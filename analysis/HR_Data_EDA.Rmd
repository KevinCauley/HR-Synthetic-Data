...
likert_outlier_files <- purrr::map_chr(likert_vars, function(var){
  ...
  p <- ggplot(hr_df, aes(x = "", y = .data[[var]])) +
    ...
    labs(
      title = paste("Outliers in", clean_label(var)),
      x = NULL,
      y = "Value"
    )

  ggsave(filename = filepath, plot = p, width = 10, height = 5.5, dpi = 300, bg = "white")
...
ggsave(filename = file.path(FIG_DIR, "correlation_matrix.png"),
       plot = p, width = 10, height = 5.5, dpi = 300, bg = "white")
...
```{r 08-numeric-skewness-calculation, echo=TRUE}
