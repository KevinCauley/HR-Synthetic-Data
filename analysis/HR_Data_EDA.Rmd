...
# Plot directory helper
create_plot_dir <- function(folder){

  base <- file.path("outputs","plots")
...
# Pre-create plot folders
create_plot_dir("continuous")
create_plot_dir("likert")
create_plot_dir("numeric_outliers")
create_plot_dir("ordered")
create_plot_dir("categorical")
create_plot_dir("targets")
...
likert_outlier_files <- purrr::map_chr(likert_vars, function(var){
  ...
  p <- ggplot(
    ...
    labs(
      title = paste("Outlier Inspection:", clean_title(var)),
      x = NULL,
      y = "Score"
    ) +
...
### Numeric (Ordered) Variables

```{r 04-ordered-variable-distributions, echo=TRUE, cache=FALSE}

plot_dir <- create_plot_dir("ordered")

ordered_plot_files <- purrr::map_chr(likert_vars, function(var){

  filepath <- file.path(
    plot_dir,
    paste0("ordered_", var, ".png")
  )
...
```{r 05-correlation-strength-distribution, fig.width=8, fig.height=5}

corr_values <- corr_matrix[lower.tri(corr_matrix)]

corr_df <- data.frame(Correlation = corr_values)
...
```{r 03-strong-correlations, fig.width=10, fig.height=8, echo=TRUE}
# Create output directory for report figures
fig_dir <- "outputs/figures"
...
# Keep unique variable pairs and compute absolute correlation
corr_df <- corr_df %>%
  filter(as.character(Var1) < as.character(Var2)) %>%
  mutate(
    Abs_Correlation = abs(Correlation),
    Pair = paste(clean_title(as.character(Var1)),
                 "vs",
                 clean_title(as.character(Var2)))
  ) %>%
  arrange(desc(Abs_Correlation))

# Keep top strongest correlations
top_corr_df <- corr_df %>%
  slice_head(n = 20) %>%
  mutate(
...
p <- ggplot(top_corr_df, aes(x = Pair, y = Correlation, fill = Direction)) +
  ...
  scale_y_continuous(
    limits = c(
      min(top_corr_df$Correlation) - 0.08,
      max(top_corr_df$Correlation) + 0.08
    )
  ) +
...
```{r 06-categorical-distributions, echo=TRUE, cache=FALSE}
...
```{r 10-logistic-preview, fig.width=10, fig.height=7}
...
model_df <- hr_df %>%
  select(all_of(num_vars), Turnover_Intention)

# Convert target
model_df$Turnover_Intention <- as.factor(model_df$Turnover_Intention)
...
p <- ggplot(
  coef_df,
  aes(x = reorder(Variable, estimate), y = estimate, fill = Direction)
) +
  geom_col(width = 0.7) +
  coord_flip() +
  ...
  scale_fill_manual(
    values = c(
      "Increases Turnover Risk" = "#C0392B",
      "Decreases Turnover Risk" = "#2E86C1"
    )
  ) +
  labs(
    title = "Preliminary Predictors of Turnover Intention",
    subtitle = "Logistic regression coefficient direction and magnitude",
    x = NULL,
...
if(!file.exists(fig_path)){
  ggsave(fig_path, p, width = 11, height = 8, dpi = 300, bg = "white")
}
...
These findings support the use of predictive modeling techniques to examine employee turnover dynamics. The next stage of analysis focuses on building machine learning models to predict **Turnover Intention**, while also exploring related behavioral outcomes such as **Quiet Quitting** and **Monitoring Turnover Risk**.
