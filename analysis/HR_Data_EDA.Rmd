...
# =====================================================
# GLOBAL OUTPUT PATHS (PROJECT ROOT SAFE)
# =====================================================

OUTPUT_ROOT <- here::here("outputs")
...
# =====================================================
# DATA PIPELINE: RAW → JOIN → CLEAN → SAVE
# This section simulates a real-world ETL process
# =====================================================
...
```{r 03-variable-validation, echo=TRUE}
cat("=== validating against these columns ===\n")
print(names(hr_df))
  ...
  purrr::map_chr(function(data){
    ...
    p <- ggplot(data, aes(x = Value)) +
      geom_histogram(aes(y = after_stat(density)),
                     ...
                     color = "white",
                     alpha = 0.9) +
      geom_density(color = "#8B0000", linewidth = 1.3) +
      labs(title = paste("Distribution of", clean_title(var))) +
      theme_minimal()

    ggsave(filepath, p, width = 9, height = 5.5, dpi = 200, bg = "white")

    normalizePath(filepath, winslash = "/", mustWork = FALSE)
  })

slickR(continuous_hist_files, slideType = "img", width = "900px") +
...
slickR(likert_plot_files, slideType = "img", width = "900px") +
  settings(dots = TRUE, arrows = TRUE, infinite = FALSE)
```
...
likert_outlier_files <- purrr::map_chr(likert_vars, function(var){
  ...
  p <- ggplot(hr_df, aes(x = "", y = .data[[var]])) +
    ...
    labs(title = paste("Outliers:", clean_title(var))) +
    theme_minimal()

  ggsave(filepath, p, width = 7, height = 5, dpi = 200, bg = "white")

  normalizePath(filepath, winslash = "/", mustWork = FALSE)
})

slickR(likert_outlier_files, slideType = "img", width = "900px") +
  settings(dots = TRUE, arrows = TRUE, infinite = FALSE)
```

### Ordered Variables

```{r 04-ordered-variables, echo=TRUE, cache=FALSE}
plot_dir <- create_plot_dir("ordered")

ordered_plot_files <- purrr::map_chr(likert_vars, function(var){

  filepath <- file.path(plot_dir, paste0("ordered_", var, ".png"))
  ...
  p <- hr_df %>%
    ...
    geom_col(fill = "#4C72B0") +
    geom_text(aes(label = n), vjust = -0.3) +
    labs(title = paste("Ordered:", clean_title(var))) +
    theme_minimal()

  ggsave(filepath, p, width = 9, height = 6, dpi = 200, bg = "white")

  normalizePath(filepath, winslash = "/", mustWork = FALSE)
})

slickR(ordered_plot_files, slideType = "img", width = "900px") +
  settings(dots = TRUE, arrows = TRUE, infinite = FALSE)
```
...
p <- ggcorrplot(
  ...
  labs(title = "Correlation Matrix of Numeric Workforce Variables") +
  theme_minimal()
...
p <- GGally::ggpairs(sample_vars)

fig_path <- file.path(FIG_DIR, "pairwise_relationships.png")
ggsave(fig_path, p, width = 12, height = 10, dpi = 300, bg = "white")
...
corr_df <- as.data.frame(as.table(corr_matrix)) %>%
  filter(Var1 < Var2) %>%
  mutate(
    Abs_Correlation = abs(Freq),
    Pair = paste(clean_title(Var1), "vs", clean_title(Var2))
  ) %>%
...
p <- ggplot(top_corr_df, aes(x = reorder(Pair, Freq), y = Freq)) +
  geom_col(fill = "#4C72B0") +
  coord_flip() +
  theme_minimal() +
  labs(title = "Top Correlations")
...
p <- ggplot(target_balance, aes(x = value, y = prop)) +
  geom_col(fill = "#4C72B0") +
  facet_wrap(~name) +
  theme_minimal()
...
p <- ggplot(hr_df, aes(x = department, fill = turnover_intention)) +
  geom_bar(position = "fill") +
  theme_minimal()

fig_path <- file.path(FIG_DIR, "turnover_by_department.png")
ggsave(fig_path, p, width = 10, height = 6, dpi = 300, bg = "white")
...
p <- ggplot(vif_df_cat, aes(x = reorder(Variable, -VIF), y = VIF)) +
...
p <- ggplot(vif_df_cat_updated, aes(x = reorder(Variable, VIF), y = VIF)) +
  geom_col(fill = "#4C72B0") +
  coord_flip() +
  theme_minimal()
...
p <- ggplot(skew_df, aes(x = Skewness, y = Variable)) +
...
p <- ggplot(skew_plot_df, aes(x = Skewness, y = Variable)) +
...
slickR(cat_plot_files, slideType = "img", width = "900px") +
  settings(dots = TRUE, arrows = TRUE, infinite = FALSE)
```
...
p <- ggplot(cat_imbalance_df, aes(x = Most_Common_Proportion, y = Variable)) +
  geom_col(fill = "#3F7F6F") +
  theme_minimal()
...
```{r 10-logistic-preview-target-var, echo=FALSE}
if(!"turnover_intention" %in% names(hr_df)){
  stop("turnover_intention missing before model")
}
