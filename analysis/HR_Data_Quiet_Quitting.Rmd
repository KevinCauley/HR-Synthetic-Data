---
title: "Quiet Quitting Modeling | Workforce Analytics"
...
# -------------------------------------------------------------------
# Gold-standard selection is based on cross-validated F1 Score, but the
# selected model must also produce usable positive-class predictions on
# the holdout test set.
#
# Quiet quitting is a behavioral and perception-based outcome, so this
# selection logic avoids over-claiming when strong cross-validation
# performance does not fully transfer to unseen data.
# -------------------------------------------------------------------

# Minimum holdout requirements for a model to be considered strongly stable
min_holdout_f1 <- 0.40
...
if (!any(selection_pool$viable_holdout, na.rm = TRUE)) {
  stop(
...
# Selection rule:
# 1. Prefer strongest CV F1 among holdout-stable configurations.
# 2. If none meet stability thresholds, select the strongest viable holdout performer.
if (any(selection_pool$stable_holdout, na.rm = TRUE)) {

  top_model_cv <- selection_pool %>%
    dplyr::filter(stable_holdout) %>%
    dplyr::arrange(dplyr::desc(cv_f1), dplyr::desc(holdout_f1)) %>%
    utils::head(1)

  selection_rule_note <- "The selected model met the holdout stability screen and ranked strongly by cross-validated F1 Score."
  selection_strategy_label <- "Holdout-Stable Risk Prioritization"

} else {

  top_model_cv <- selection_pool %>%
    dplyr::filter(viable_holdout) %>%
    dplyr::arrange(dplyr::desc(holdout_f1), dplyr::desc(cv_f1)) %>%
    utils::head(1)

  selection_rule_note <- "No configuration met the full holdout stability screen, so the selected model is the strongest viable holdout performer among cross-validated candidates."
  selection_strategy_label <- "Holdout-Viable Risk Prioritization"
}
...
# Build holdout metric table for heatmap
plot_df <- results_all %>%
  filter(.metric %in% c("precision", "recall", "f_meas", "roc_auc")) %>%
  mutate(
    Metric = factor(
...
p <- final_preds %>%
  ...
  geom_vline(
    xintercept = moderate_threshold,
    linetype = "dashed",
    linewidth = 0.7,
    color = "#F39C12"
  ) +
  geom_vline(
    xintercept = high_threshold,
    linetype = "dashed",
    linewidth = 0.7,
    color = "#C0392B"
  ) +
  ...
  labs(
    title = "Predicted Quiet Quitting Risk Distribution",
    subtitle = "Employees are scored by their predicted probability of quiet quitting intention",
